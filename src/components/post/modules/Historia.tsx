...
import { HistoriaPub, HistoriaPubProps } from "../types/post.types";
import Image from "next/legacy/image";
import moment from "moment";
import createProfilePicture from "@/lib/helpers/createProfilePicture";

const Historia: FunctionComponent<HistoriaPubProps> = ({
  historia,
  dict,
  votosCargando,
}): JSX.Element => {
  return (
    <div className="relative w-full flex items-start justify-start h-fit flex-col gap-2 pt-10">
      ...
      {votosCargando ? (
        ...
        <div className="relative w-full h-fit grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {historia?.map((hist: HistoriaPub, indice: number) => {
